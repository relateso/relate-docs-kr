---
title: "임포트 하는 법"
description: "AI로 고객 데이터를 빠르게 Relate으로 가져올 수 있습니다."
sidebarTitle: "임포트 하는 법"
"og:title": "임포트 하는 법 | Relate Product Guide"
"og:description": "AI로 고객 데이터를 빠르게 Relate으로 가져올 수 있습니다."
...
"twitter:description": "AI로 고객 데이터를 빠르게 Relate으로 가져올 수 있습니다."
"twitter:image": "https://raw.githubusercontent.com/relateso/relate-docs-kr/main/docs/images/setup/import-main.png"
---
## Overview
CSV/Excel 파일을 업로드하면 Relate AI가 데이터의 의미를 자동으로 이해해 기존 고객 데이터를 Relate에 쉽고 간편하게 임포트할 수 있습니다.
...
<Accordion title="1. 임포트 데이터 종류">
  ...
  Relate 내에서 고객 정보를 분류할 때 **조직이 가장 기본**이 되며, 연락처는 조직 내에 속하게 되어 있습니다.
...
<Accordion title="2. 데이터 필드 (Data fields)">
  ...
  3. 프로세스 / 리스트 : 프로세스와 리스트의 [엔트리(Entry)](/features/processes#1-entry) 타입(조직 혹은 연락처)을 확인해 1번 혹은 2번을 따릅니다.
...
<Accordion title="3. 커스텀 필드 (Custom fields)">
  ...
  업로드한 파일의 컬럼을 AI가 자동으로 해석하여, **기존 데이터 필드로 매핑할 수 없는 정보가 있는 경우에는 필요한 커스텀 필드를 자동으로 생성하여 매핑**합니다. 
...
## 임포트 실행 가이드 (Step-by-Step)
  ...
  - Relate 앱 화면에서 좌측 상단을 클릭하여 `워크스페이스 설정`을 선택합니다.
  <img src="/images/setup/ws-select.png" alt="워크스페이스 선택" width="300" style={{ display: "block", margin: "4px 0 12px 32px", boxShadow: "0 6px 18px rgba(0, 0, 0, 0.16)", borderRadius: "8px" }} />

    <span style={{ display: "block", height: "14px" }} />
  - 좌측 사이드바의 `연락처 파일 업로드`를 선택한 후, 우측 상단의 `Import contacts`를 클릭합니다.
    <img
  ...
  <img
    src="/images/setup/ai-import-duplicate-handling.png"
  ...
  <Tip>
    ...
    <div style={{ display: "grid", gap: "4px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
        ...
        <span>Organization Field로 선택한 컬럼은 **조직**으로 임포트됩니다.</span>
      </div>
    </div>
  </Tip>
...
#### 컬럼 매핑 변경하기   
  ...
  - 매핑할 필드가 없는 경우, 우측 상단의 `커스텀 필드 추가`를 통해 새로운 필드를 생성해 바로 매핑할 수도 있습니다.
      ...
      <img
        src="/images/setup/add-custom-field.png"
...
<img
  src="/images/setup/import-result.png"
  alt="import-result"
  style={{
    display: "block",
    margin: "0 auto",
...
연락처와 조직 데이터를 임포트하면서 특정 프로세스 또는 리스트에 바로 추가하고 싶다면, 해당 프로세스/리스트에서 CSV 임포트를 진행할 수 있습니다.

  ##### **Step 1. 프로세스/리스트 선택 또는 생성**

    기존에 존재하는 프로세스/리스트를 선택하거나, 새로 생성합니다.

    <img
      src="/images/setup/makeprocess.png"
      alt="make process"
      width="300"
      style={{
    ...
    각 프로세스/리스트는 Contact(연락처) 또는 Organization(조직) 중 하나의 **엔트리 타입**을 가지며, 임포트할 데이터 유형에 맞는 프로세스/리스트를 선택합니다.

  ##### **Step 2. CSV 임포트 선택**
    
    임포트하고자 하는 프로세스/리스트 상단의 `+ New contact` 혹은 `+ New organization` 버튼을 클릭합니다.
    <img
      src="/images/setup/process-imp.png"
      alt="process import"
      width="360"
      style={{
  ...
  ##### **Step 3. 임포트 진행**
    이후 과정은 앞서 안내한 임포트 방법과 동일하게 진행되며, 임포트가 완료되면 각 행이 하나의 엔트리로 생성되어 프로세스/리스트에 바로 추가됩니다.
...
<Accordion title="기존 임포트 방식과 어떤 차이가 있나요?">
  기존 임포트 방식에서는 임포트 이전에 수동으로 커스텀 필드를 생성해둔 후 CSV 구조에 맞춰 사용자가 직접 컬럼을 매핑하여 임포트를 해야 했습니다.
   ...
   이제는 파일을 업로드하면 **Relate AI가 컬럼의 의미를 자동으로 이해해 매핑하며, 사전에 별도의 필드 설정 없이도 임포트를 완료할 수 있습니다.**
</Accordion>
