...
AI 임포트는 CSV/Excel 파일을 업로드하면,  Relate AI가 데이터의 의미를 이해해 연락처/조직을 자동으로 추가하거나 업데이트하는 임포트 방식입니다. 
...
<Accordion title="2. 데이터 필드 (Data fields)">
  ...
  3. 프로세스 / 리스트 : 프로세스와 리스트의 [Entry](/features/processes#1-entry) 타입(조직 혹은 연락처)을 확인해 1번 혹은 2번을 따릅니다.
...
<Accordion title="기존 임포트 방식과 어떤 차이가 있나요?">
  ...
  AI 임포트는 파일을 업로드하면 **Relate AI가 컬럼의 의미를 자동으로 이해해 매핑하며, 사전에 별도의 필드 설정 없이도 임포트를 진행**할 수 있다는 점이 가장 큰 차이입니다.
</Accordion>
